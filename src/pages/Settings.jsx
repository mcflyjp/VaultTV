...
function LoginPanel({ signInWithEmail, signUpWithEmail, signInWithGoogle }) {
  ...
  return (
    <div>
      ...
      {IS_FIRETV ? (
        // Google blocks OAuth in embedded WebViews — show a warning instead
        // The button still works (opens Silk browser) but email/password is easier
        <div style={{ maxWidth: 340, padding: '0.65rem 0.85rem', background: 'rgba(255,255,255,0.05)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
            💡 <strong style={{ color: 'var(--text-primary)' }}>Tip:</strong> Use email + password above for the easiest sign-in on FireTV.
            Google sign-in will open the Silk browser — after completing, return here and refresh.
          </p>
          <button
            onClick={signInWithGoogle}
            style={{ marginTop: '0.65rem', display: 'flex', alignItems: 'center', gap: '0.6rem', background: '#fff', color: '#111', border: 'none', borderRadius: 'var(--radius)', padding: '0.5rem 1rem', cursor: 'pointer', fontWeight: 600, fontSize: '0.82rem' }}
          >
            ...
            Open Google sign-in in Silk browser
          </button>
        </div>
      ) : (
        <button
