...
Self-hosted media server. Runs on your PC, serves the full VaultTV web app at `http://your-pc-ip:7842` so any device on your network (or the internet, if you port-forward) can stream your local library from a browser.
...
```
This installs dependencies, builds the web app, and starts the server. Then open **http://localhost:7842** in your browser and set your admin password.
...
### Same network (LAN)
Open `http://YOUR-PC-IP:7842` on any device on your home network. Find your PC's IP in Settings → Network → Properties (look for IPv4 Address, e.g. `192.168.1.100`).
...
3. Add a rule: **External port 7842 → Internal IP (your PC) → Internal port 7842**
4. Find your public IP at [whatismyip.com](https://www.whatismyip.com)
5. Access VaultTV at `http://YOUR-PUBLIC-IP:7842`

> ⚠️ Port forwarding exposes the server to the internet. The admin password protects it — make it strong.

### Dynamic DNS (optional)
If your public IP changes, use a free DDNS service like [DuckDNS](https://www.duckdns.org) to get a stable hostname (e.g. `yourname.duckdns.org:7842`).
